...
// ==========================================
// 1. ZASZYFROWANA KONFIGURACJA CHMURY
// ==========================================
// Nazwa bucketu ("galeria-marcina") została zaszyfrowana algorytmem AES przy użyciu Twojego tajnego hasła.
// Dzięki temu nikt nie podejrzy oryginalnej nazwy bucketu ani adresu URL w kodzie źródłowym przed zalogowaniem.
const ENCRYPTED_BUCKET_NAME = "U2FsdGVkX19P9Mv0R7QY2Z/vN17XzGg7pIsVepD2K6u9qscTclqUaY1A5/4eW4Lh"; 
...
function App() {
  ...
  // ==========================================
  // 2. DESZYFROWANIE I POBIERANIE LISTY PLIKÓW
  // ==========================================
  const checkAnswerAndFetch = async () => {
    // Normalizacja wpisanej odpowiedzi (małe litery, brak spacji)
    ...
    try {
      // Próbujemy odszyfrować nazwę bucketu wpisanym hasłem
      const decryptedBytes = CryptoJS.AES.decrypt(ENCRYPTED_BUCKET_NAME, preparedAnswer);
      const decryptedBucketName = decryptedBytes.toString(CryptoJS.enc.Utf8);

      // Jeśli wpisano złe hasło, deszyfrowanie zwróci pusty ciąg znaków
      if (!decryptedBucketName || decryptedBucketName !== "galeria-marcina") {
        throw new Error("Błędne hasło");
      }

      // Generujemy poprawne adresy URL dynamicznie na podstawie odszyfrowanej nazwy
      const bucketUrl = `https://f005.backblazeb2.com/file/${decryptedBucketName}`;

      // Pobieramy listę plików z API
      const response = await fetch('/api/gallery');
      if (!response.ok) throw new Error("Problem z pobraniem danych z serwera proxy");
      ...
      keys.forEach(key => {
        ...
        if (parts.length > 1) {
          ...
          const fullUrl = `${bucketUrl}/${encodedKey}`;
      ...
      // Dopiero gdy deszyfrowanie i pobieranie danych zakończyło się sukcesem, pokazujemy galerię
      setShowGallery(true);

    } catch (error) {
      console.error("Autoryzacja nieudana lub błąd pobierania:", error);
      ...
      <div className="main-wrapper">
        ...
        <div className="profile-card">
          ...
          <div className="gallery-login">
            ...
            <input 
              type="password" 
              placeholder={loading ? "Autoryzacja..." : "Odpowiedź..."}
              disabled={loading}
              value={answer}
